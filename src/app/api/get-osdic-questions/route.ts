...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const randomizedQuestions = questions.slice(0,30).map((q, index) => ({
      slNo: index + 1,
      ...q,
    }));

    const selectedQuestionIds = new Set(randomizedQuestions.map((q) => q.question_id));
    const filteredConf = Object.fromEntries(
      Object.entries(OSDICAnswers).filter(([key]) =>
        selectedQuestionIds.has(parseInt(key)),
      ),
    );
